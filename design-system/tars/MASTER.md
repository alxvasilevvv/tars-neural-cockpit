...
| HUD-class mono labels (phase bar, status bar, source chip, kbd hint, policy gate header, live-rail `stream-head` / `integrity-head`) | **Share Tech Mono / Fira Code** 500 | **W308 step 4 → W309 prep**: `--font-size-hud-mono: 11px` (was implicit `10px` at all six call-sites). Share Tech Mono / Fira Code lose letterform clarity at 10px even on Retina (caps look rectangular, `I`/`l` ambiguous); 11px keeps the HUD-tick feel and stays inside the meta row. Originally landed step 4 as `--font-size-phase-bar` for just the watch-me-work bar; renamed + broadened in the W309-prep wave when the other five call-sites (`.source-chip`, `.gate-head`, `.send-kbd`, `.status-bar`, `.stream-head`, `.integrity-head`) migrated onto the same token. Single drift contract: no hardcoded `10px` / `11px` allowed on any element using `font-family: var(--font-mono)`. |
