...
async function fetchAsset(
  ...
  // Primary path — release directly attached to the tag.
  ...
  if (byTag.ok) {
    ...
    if (release && Array.isArray(release.assets)) {
      const hit = release.assets.find((a) => a.name === filename);
      if (hit) return hit;
    }
  }

  // Wave 142 fallback — when CI runs are cancelled mid-publish (or
  // when the tag/release association drifted), the by-tag lookup
  // returns a release with no assets while the actual binaries live
  // on a sibling draft/untagged release named "TARS v<tag>". List
  // ALL releases (incl. drafts) and find the first that has the
  // requested filename, preferring ones whose name/tag matches the
  // requested tag.
  ...
  const releases = (await list.json()) as GithubRelease[] | null;
  if (!Array.isArray(releases)) return null;

  // Prefer releases whose tag_name or name mentions our target tag.
  const tagBare = tag.replace(/^v/, "");
  const matchesTag = (r: GithubRelease & { name?: string }) =>
    r.tag_name === tag ||
    (r.name && r.name.toLowerCase().includes(tagBare));

  const ranked = [
    ...releases.filter(matchesTag),
    ...releases.filter((r) => !matchesTag(r)),
  ];
