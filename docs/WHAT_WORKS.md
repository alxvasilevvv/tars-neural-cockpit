...
| Background TARS daemon — real macOS LaunchAgent + heartbeat *(Wave 152)* | `backend/core/daemon/`, `scripts/tars-daemon`, `tests/test_daemon.py`, `docs/contracts/BACKGROUND_DAEMON.md` |
...
| Background TARS daemon (macOS LaunchAgent + heartbeat) *(Wave 152)* | Real `launchd` LaunchAgent. `tars-daemon install` writes `~/Library/LaunchAgents/com.tars.background.plist`, bootstraps via `launchctl bootstrap gui/<uid>`, KeepAlive on crash. Heartbeat at `~/.tars/daemon.heartbeat`. macOS only — Linux systemd v9.1.3, Windows v9.2. | `backend/core/daemon/{__init__,runner,launchd,__main__}.py`, `scripts/tars-daemon`, `docs/contracts/BACKGROUND_DAEMON.md` |
