...
> `W308_PRE_FLIGHT_FINDINGS.md`: rename `--font-size-phase-bar` →
> `--font-size-hud-mono` and migrate every HUD-class mono label to
> it. Functional W309 (mic / WS / conversation) **still gated on a
> separate operator OK per plan E** — this wave is purely the design
> tightening that makes the field clean before that lands.
>
> **Token contract:** `--font-size-hud-mono: 11px` is now the *single*
> token for the whole HUD-class mono label family. MASTER §4
> typography row rewritten to make the scope explicit (phase bar,
> status bar, source chip, kbd hint, policy gate header, live-rail
> `stream-head` / `integrity-head`). Single drift contract: "no
> hardcoded `10px` / `11px` allowed on `font-family: var(--font-mono)`
> blocks". Originally landed step 4 as `--font-size-phase-bar` because
> that was the only call-site the W307 verdict named by symbol; the
> rename is non-breaking and just widens the contract to where it
> always should have been.
>
> **Call-sites migrated (7 total on one token):** `cockpit.html` —
> `.phase-bar`, `.source-chip`, `.gate-head`, `.send-kbd`, `.status-bar`;
> `hero.html` — `.stream-head`, `.integrity-head`. All previously
> hardcoded `10px` except `.phase-bar` which already pointed at the
> step-4 token.
>
> **Drift suite:** `test_phase_bar_size_token_declared_and_applied`
> renamed and broadened to `test_hud_mono_font_size_token_declared_and_applied`
> (asserts declaration + 11px + MASTER row + ≥5 cockpit + ≥2 hero
> usages). New `test_no_hardcoded_pixel_size_on_mono_family_elements`
> walks every `apps/cockpit/*.html`, parses CSS `{ … }` blocks, and
> fails on any block declaring both `font-family: var(--font-mono)`
> and a hardcoded `font-size: 10px` / `11px`. Diagnostic includes
> `file:line + 240-char block preview`. Suite total: 10 → **11**.
> Negative-control: re-injecting a 10px hardcode in `.source-chip`
> makes the new guard fail at `cockpit.html:434` with the expected
> diagnostic — confirms not vacuous.
>
> **Verification.** `pytest tests/test_cockpit_tokens_sync.py -v` →
> 11 passed in 0.03s. `bash desktop/scripts/package-cockpit.sh` →
> OK; built bundle has 5× `var(--font-size-hud-mono)` in `cockpit.html`,
> 2× in `hero.html`, zero hardcoded `10px` mono blocks. Cockpit
> bundle stable at ~27 kB raw / ~6 kB gzipped.
>
> ---
>
> **W308 step 4 (previous wave)** — operator delegated ("делай всё
