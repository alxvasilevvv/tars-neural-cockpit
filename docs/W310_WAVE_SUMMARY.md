...
**State:** 37 PRs open awaiting operator merge (+1 infra PR **W310-au** for the remaining 3 stale CI workflows). PR **#188** also ships **W310-at sync layer** (`CURRENT_STATUS.md` + `TARS_MASTER_DOC.md §6.4` + `PROJECT_INDEX.md` reconciled). 5 docs-only extensions to PR #192 cover the entire operator surface from D-0 (today) → D+365 (v11 GA). All 6 verdict wrappers shipped + spec-pinned. Cross-stack mirror with brother on PR #198 §8.A.

**What the operator needs to do (in order):**

1. **Today** — merge PR **#188** first (root: fixes `qa-agent.yml` workflow-cache staleness per W310-c + ships W310-at state-doc sync). Merge **W310-au** CI PR in parallel or immediately after (#188 fixes `qa-agent.yml`; W310-au fixes `e2e-suite.yml`, `eval-suite.yml`, `credential-sentinel.yml` — same header-comment trick, file-independent). Then paste-once W310-an merge sequence to land the remaining 36 PRs in ~20-30 min wall-clock.
...
**Pickup pointers (6 docs to read on agent landing):**
1. **`docs/W310_WAVE_SUMMARY.md`** (this doc) — single-page W310 retrospective + 5 playbooks
2. **`CURRENT_STATUS.md`** (PR #188, W310-at) — daily one-pager: 8 hard GA blockers + 6-wrapper paste playbook
3. `docs/PRODUCT_MASTER_PLAN.md` (PR #188) — forward execution plan post-v10
4. `docs/handoff/PH11_QA_SWEEP_BRIEF.md` (PR #197) — GA dock-down methodology
5. `docs/handoff/PH11_BROTHER_HANDOFF_BRIEF.md` (PR #198) — brother coord + §8.A first-week mirror
6. `docs/handoff/PH4_APPLE_SIGN_V10_BRIEF.md` (PR #199) — Apple sign hard blockers B1-B5

**For full detail, keep reading. This TLDR captures the state at W310-at/au (2026-05-19). End of TLDR.**
...
| **W310-c** | CI infrastructure diagnosis — root cause of `qa-agent.yml` failures traced to GH Actions workflow-registration cache staleness since 2026-05-13 | Header-comment touch baked into PR #188 (`qa-agent.yml`); **W310-au** ships the same trick for `e2e-suite.yml`, `eval-suite.yml`, `credential-sentinel.yml` on branch `cursor/w310-ci-workflow-cache-refresh` (file-independent infra PR, merge anytime after or parallel with #188) |
...
| **W310-at** | **State-doc sync layer on PR #188** — closes silent drift between forward-execution plan, daily pulse, master doc, and project index after W310-as landed | `CURRENT_STATUS.md` refresh (2026-05-15/W269 → post-W310-as: TLDR pointer, 8 hard GA blockers, 6-wrapper paste playbook); `TARS_MASTER_DOC.md §6.4` 'Reconciliation (W310-at)' appended after W267 Top-5 historical snapshot; `PROJECT_INDEX.md` adds missing `MCP_REWRITE_BRIEF.md` entry; `docs/CHANGELOG_AGENTS.md` entry prepended. Three canonical state docs synced into one merge on PR #188 |
| **W310-au** | **CI workflow cache refresh (infra)** — extends W310-c header-comment fix to the 3 remaining stale workflows | **PR #224** (`cursor/w310-ci-workflow-cache-refresh`): header-comment touches on `.github/workflows/e2e-suite.yml`, `eval-suite.yml`, `credential-sentinel.yml` (fixes cosmetic 2-3 s fails on `TARS B2B E2E suite`, `TARS eval suite`, `scan working tree`). File-independent of PR #188; merge anytime after or parallel with #188 |
...
> registration cache that went stale. **W310-c / W310-au fix:**
> PR #188 includes the header-comment touch for `qa-agent.yml`;
> **W310-au** (`cursor/w310-ci-workflow-cache-refresh`) applies the
> same trick to `e2e-suite.yml`, `eval-suite.yml`, and
> `credential-sentinel.yml` (the job behind `scan working tree`).
> Merge #188 first (or W310-au in parallel — file-independent).
> **None of these failures reflect actual test results.**
...
# Tier 0b (optional, parallel-safe): W310-au CI workflow cache refresh
# — fixes e2e-suite / eval-suite / credential-sentinel cosmetic fails.
# File-independent of #188; can merge before, after, or in parallel.
# gh pr merge <W310-au-pr-number> --squash --delete-branch
