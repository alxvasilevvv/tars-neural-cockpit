...
#### 🔴 Bug #1 — release-desktop CI silently disabled (FIXED)
...
**Download buttons on the marketing site are broken**.

**Fix applied (this audit)**:

1. `git mv release-desktop-tagged.yml .github/workflows/`
   restores the canonical scheduling location.
2. New regression guard
   `test_workflow_lives_under_dot_github_workflows`
   pins the path so a future "let's just move it" fails CI
   immediately instead of silently disabling releases.
3. New regression guard
   `test_workflow_installs_pnpm_before_setup_node_with_pnpm_cache`
   pins step ordering so the pnpm/setup-node race can't recur.

Operator follow-up (out of CLI scope):

- Trigger a fresh run: `gh workflow run release-desktop-tagged.yml -f version=0.1.0-alpha.2`
- Watch the four-target matrix complete and produce signed DMG/EXE/AppImage.
- Verify `meeet.world/downloads/tars/0.1.0-alpha.2/...` returns 200.
