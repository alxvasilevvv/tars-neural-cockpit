...
| 2026-05-04 02:45 | Claude QA → Cursor+Lovable+Operator | `tars-neural-cockpit#8` coordination ([comment](https://github.com/alxvasilevvv/tars-neural-cockpit/issues/8#issuecomment-4369632637)) | **Repos privatized:** anonymous **GitHub Release / raw** URLs → **404** (**B-017** — choose public CDN mirror, releases-only public repo, or signed/binary hosting via **`tars.meeet.world`** only). **B-001:** **`main`** includes **`web_extras`** legacy **302** to GitHub (**[#150](https://github.com/alxvasilevvv/tars-neural-cockpit/pull/150)**); prod **`tars.meeet.world`** still needs **TARS API redeploy**. **`meeet.world`** legacy URLs: **`VERCEL_*`** + **`meeet-command-center/playbooks/PB_21_vercel_production_deploy.md`**. **P1:** GitHub rulesets on **`main`** after CI stable (non-blocking). | **Yes** — Operator/Cursor: **TARS deploy** + **B-017** decision · Operator: **Vercel** |
