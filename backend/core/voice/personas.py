...
class PersonaProviderHint:
    ...
    NOTE: Operator runs on the ElevenLabs *free* tier. Cinematic delivery
    is unlocked **only** via aggressive parameter tuning (free tier blocks
    Professional Voice Clones, v3 alpha, and most starter voices read flat
    at default settings). The ranges below reflect what actually delivers
    "living, character-driven" output on free-tier starter voices:

    - ``stability`` 0..1 — lower = more emotive / varied prosody,
      higher = monotone but consistent. **0.18–0.30 for charismatic
      personas (Stark, GLaDOS, Jarvis), 0.40–0.50 for measured
      personas (TARS, Operator), 0.75–0.85 for clinical (HAL).**
    - ``similarity_boost`` 0..1 — keeps the rendered audio close to
      the original speaker. **0.88–0.92 for cinematic lock** —
      below 0.85 the voice drifts into stock TTS narrator.
    - ``style`` 0..1 — exaggerates the speaker's idiosyncratic
      delivery (only honoured by ``eleven_multilingual_v2`` and
      newer). **0.55–0.88 for charismatic personas**, 0.05–0.30 for
      clinical / neutral. Going above 0.90 starts to glitch on some
      voices.
    ...
    personas = {
        "jarvis": Persona(
            ...
            provider=PersonaProviderHint(
                ...
                elevenlabs_stability=0.22,    # pushed lower → much more living prosody
                elevenlabs_similarity=0.92,   # high identity lock
                elevenlabs_style=0.78,        # cinematic butler — heavy character
        ...
        "stark": Persona(
            ...
            provider=PersonaProviderHint(
                ...
                elevenlabs_stability=0.18,    # extreme low → swaggering, alive delivery
                elevenlabs_similarity=0.88,   # tighter identity lock
                elevenlabs_style=0.88,        # max character for cocky punchlines
        ...
        "hal9000": Persona(
            ...
            provider=PersonaProviderHint(
                ...
                elevenlabs_stability=0.82,    # max stability → uncanny calm monotone
                elevenlabs_similarity=0.92,
                elevenlabs_style=0.05,        # near-zero style → clinical detachment
        ...
        "glados": Persona(
            ...
            provider=PersonaProviderHint(
                ...
                elevenlabs_stability=0.26,    # extreme low → biting prosody swings
                elevenlabs_similarity=0.90,
                elevenlabs_style=0.85,        # near-max sarcasm character
        ...
        "tars": Persona(
            ...
            provider=PersonaProviderHint(
                ...
                elevenlabs_stability=0.42,    # tighter measured cadence, less drift
                elevenlabs_similarity=0.92,   # locked deep-baritone identity
                elevenlabs_style=0.55,        # dry humour edge without theatrics
        ...
        "operator": Persona(
            ...
            provider=PersonaProviderHint(
                ...
                elevenlabs_stability=0.40,    # warmer, more present
                elevenlabs_similarity=0.88,
                elevenlabs_style=0.30,        # subtle warmth without character bleed
