"""``python -m backend.core.mcp.client_pkg`` — operator CLI.

Useful for inspecting a remote MCP server from the terminal
without writing Python:

    # List configured remote servers
    python -m backend.core.mcp.client_pkg list-servers

    # List tools on one server
    python -m backend.core.mcp.client_pkg list-tools tars-self

    # Call a tool with JSON arguments
    python -m backend.core.mcp.client_pkg call-tool tars-self \
...
def _bridge_unregister() -> int:
    from backend.core.mcp.bridge_pkg import unregister_bridges
...
def _bridge_cache_list(*, show_tools: bool) -> int:
    from backend.core.mcp.bridge_pkg import ToolCache
    from backend.core.mcp.bridge_pkg.bootstrap import _default_cache_root
...
def _bridge_pool_bench(
    *, server: str, tool: str, arguments: str, iterations: int
    ...
    from backend.core.mcp.bridge_pkg import (
        SessionPool,
        aboot_mcp_bridges,
        unregister_bridges,
    )
